...
import Foundation
import AppKit
public class DemoBox : NSView {
    
    var magnitude : Int = 0
    
    override public func draw(_ dirtyRect: NSRect) {
        let context = NSGraphicsContext.current()?.cgContext
        self.drawNormalBox(context : context!)

    }
    
    func drawNormalBox(context : CGContext)
    {
        let scale = 10
//        var baseSize = 100;
        let diff = magnitude * 2;
//        var dimension = (baseSize + scale * diff);
        context.saveGState()
        let viewWidth = self.bounds.size.width + CGFloat(scale * diff)
        let viewHeight = self.bounds.size.height + CGFloat(magnitude * 10)
        
        
        let color = NSColor.blue.cgColor
        
        context.setFillColor(color)
//        let newFrame = CGRect(x: self.frame.origin.x, y: self.frame.origin.y, width: viewWidth, height: viewHeight)
//        print(self.frame)
...
//        
//        for i in 0..<count! {
//            var magnitude = fft?[i].magnitudeAtBin
